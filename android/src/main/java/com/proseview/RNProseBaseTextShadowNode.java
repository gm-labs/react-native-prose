...
public abstract class RNProseBaseTextShadowNode extends LayoutShadowNode {
  ...
  private static void buildSpannedFromShadowNode(
          RNProseBaseTextShadowNode textShadowNode,
    ...
    for (int i = 0, length = textShadowNode.getChildCount(); i < length; i++) {
      ...
      if (child instanceof RNProseRawTextShadowNode) {
        sb.append(
                TextTransform.apply(
                        ((RNProseRawTextShadowNode) child).getText(), textAttributes.getTextTransform()));
      } else if (child instanceof RNProseBaseTextShadowNode) {
        buildSpannedFromShadowNode(
                (RNProseBaseTextShadowNode) child,
                sb,
                ops,
                textAttributes,
                supportsInlineViews,
    ...
    if (end >= start) {
      ...
      boolean shouldCreateClickableSpan = textShadowNode.hasOnPress() || textShadowNode.hasOnLongPress() || roleIsLink;

      if (shouldCreateClickableSpan) {
  ...
  protected boolean mHasOnPress = false;
  protected boolean mHasOnLongPress = false;
  ...
  public RNProseBaseTextShadowNode() {
    this(null);
  }
  ...
  public void setOnPress(boolean hasOnPress) {
    mHasOnPress = hasOnPress;
    markUpdated();
  }

  @ReactProp(name = "onLongPress")
  public void setOnLongPress(boolean hasOnLongPress) {
    mHasOnLongPress = hasOnLongPress;
    markUpdated();
  }
  ...
  public boolean hasOnLongPress() {
    return mHasOnLongPress;
  }

  public boolean roleIsLink() {
    return mRole != null ? mRole == Role.LINK : mAccessibilityRole == AccessibilityRole.LINK;
  }
